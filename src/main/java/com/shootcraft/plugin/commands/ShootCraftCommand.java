 ...
 *   /sc leaderboard <cat> [remove|size <n>] - gere un leaderboard holographique
 */
public class ShootCraftCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> SUBCOMMANDS = List.of(
            ...
            "join", "joinrandom", "leave", "start", "stop", "info", "leaderboard", "help"
    );
    ...
    /**
     * /sc leaderboard <victoires|kills|kd|parties>              - spawn a la position du joueur
     * /sc leaderboard <categorie> remove                        - supprime le leaderboard
     * /sc leaderboard <categorie> size <taille>                 - regle l'echelle (0.1 a 10)
     */
    private void handleLeaderboard(CommandSender sender, String[] args) {
        if (!requireAdmin(sender)) return;
        if (args.length < 2) {
            MessageUtil.sendKey(plugin, sender, "leaderboard-usage");
            return;
        }

        LeaderboardManager.Category category = LeaderboardManager.Category.fromKey(args[1]);
        if (category == null) {
            MessageUtil.sendKey(plugin, sender, "leaderboard-unknown-category");
            return;
        }
        ...
        if (args.length >= 3) {
            ...
            if (action.equals("remove")) {
                boolean removed = leaderboardManager.despawn(category);
                if (removed) {
                    MessageUtil.sendKey(plugin, sender, "leaderboard-removed", "category", category.getKey());
                } else {
                    MessageUtil.sendKey(plugin, sender, "leaderboard-not-active", "category", category.getKey());
                }
            } else if (action.equals("size")) {
                if (args.length < 4) {
                    MessageUtil.sendKey(plugin, sender, "leaderboard-usage-size");
                    MessageUtil.sendKey(plugin, sender, "leaderboard-size-hint");
                    return;
                }
                double size;
                try {
                    ...
                    MessageUtil.sendKey(plugin, sender, "leaderboard-size-invalid");
                    return;
                }
                if (size < 0.1 || size > 10) {
                    MessageUtil.sendKey(plugin, sender, "leaderboard-size-range");
                    return;
                }
                leaderboardManager.setScale(category, size);
                MessageUtil.sendKey(plugin, sender, "leaderboard-scale-set", "category", category.getKey(),
        ...
        if (!(sender instanceof Player player)) {
            MessageUtil.sendKey(plugin, sender, "leaderboard-player-only");
            return;
        }

        leaderboardManager.spawn(category, player.getLocation());
        MessageUtil.sendKey(plugin, sender, "leaderboard-spawned", "category", category.getKey());
    }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        if (sender.hasPermission("shootcraft.admin")) {
            ...
            MessageUtil.send(sender, "&c/sc leaderboard <victoires|kills|kd|parties> &7- Spawner un leaderboard top 10");
            MessageUtil.send(sender, "&c/sc leaderboard <categorie> remove &7- Supprimer ce leaderboard");
            MessageUtil.send(sender, "&c/sc leaderboard <categorie> size <taille> &7- Regler la taille de l'hologramme");
        }
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2) {
            ...
            if (sub.equals("leaderboard")) {
                return filter(List.of("victoires", "kills", "kd", "parties"), args[1]);
            }
        ...
        if (args.length == 3 && args[0].equalsIgnoreCase("leaderboard")) {
            return filter(List.of("remove", "size"), args[2]);
        }
